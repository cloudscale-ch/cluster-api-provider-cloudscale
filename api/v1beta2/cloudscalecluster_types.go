...
type RouterInterfaceSpec struct {
	...
	// interface. For the common case of a single interface on a managed (CIDR)
	// network, the admission webhook defaults it: the ConfigureSubnetGateway owner
	// gets the network's gatewayAddress, or network+3 if unset (cloudscale.ch
	// reserves .1 and .2 for DNS), and that value is mirrored to the network's
	// gatewayAddress. When multiple router interfaces reference the same network,
	// each must set this explicitly. Immutable after creation.
	// +optional
