...
func (r *CloudscaleClusterReconciler) reconcileFloatingIP(ctx context.Context, clusterScope *scope.ClusterScope) (reterr error) {
	...
	if fipSpec.UUID != "" {
		return r.reconcileBYOFloatingIP(ctx, clusterScope, fipSpec.UUID)
	}
...
func (r *CloudscaleClusterReconciler) reconcileBYOFloatingIP(ctx context.Context, clusterScope *scope.ClusterScope, uuid string) error {
	...
	fip, err := clusterScope.CloudscaleClient.FloatingIPs.Get(ctx, uuid)
	if err != nil {
...
func (r *CloudscaleClusterReconciler) deleteFloatingIP(ctx context.Context, clusterScope *scope.ClusterScope) (reterr error) {
	...
	// BYO floating IPs are not deleted
	if fipSpec.UUID != "" {
		clusterScope.Info("Skipping BYO floating IP deletion", "uuid", fipSpec.UUID)
		return nil
	}
